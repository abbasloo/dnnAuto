...
for i = 1:size(shape.X,1)
    ...
    try
        [in_ray,in_ring,shape,geod,directions,ds] = compute_disk(shape,i,dists,flag_dist,'scales',[0,rr],'N_rays',length(th),'fhs',fhs,'fha',fha);
        ...
        s = size(desc_net_shuffled);
        desc_net = M_ * desc;
        if norm(reshape(desc_net_shuffled, s(1), s(2)*s(3))' - desc_net) > 1e-04
